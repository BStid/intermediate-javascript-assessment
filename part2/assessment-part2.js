...
function deathStar(capacity, crew) {
  return (variable = capacity.bind(crew));
}
...
function accountingOffice(assests) {
  return function closurefunction(liabilities) {
...
function forgetter(name) {
  return function rememberall(item) {
    let rememberedItems = {
      name: name,
      remember: ["hogwartshphogsmeadquidditch"]
    };
    rememberedItems.remember.push(item);
  ...
  return {
    dinnerOverFire: function() {
      if (
        ...
        let hunger = hungerValue - 25;
        let danger = dangerValue + 40;
    ...
    hidingInBush() {
      if (
        ...
        let hunger = hungerValue + 35;
        let danger = dangerValue - 20;
        let object = {
          hunger: hunger,
          danger: danger
        };
